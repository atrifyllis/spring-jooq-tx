...
class TenantUseCaseIntegrationTest : BasePostgresTest() {
    ...
    fun `should store new tenant and user`() {
        ...
        val info = UserInfo("test_user", Companion.TEST_EMAIL_NEW, emptyList())
    ...
    fun `should not save tenant when user insertion fails`() {
        ...
        assertThrows<TenantCreationException> {
            cut.generateTenant(UserInfo("test_user", Companion.TEST_EMAIL_TOO_LONG, emptyList()))
        }
    ...
    companion object {
        private val TEST_EMAIL_TOO_LONG = "testtesttest@email.com"
        private val TEST_EMAIL_NEW = "test_new@email.com"
    }
}
